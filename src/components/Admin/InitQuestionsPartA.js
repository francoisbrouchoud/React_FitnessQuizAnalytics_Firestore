...
export default function InitQuestionsPart2()
{
    ...
    */
    ...
    async function uploadQuestions() {
        try {
            // Get a reference to the "questions" collection in Firestore
            const questionsRef = collection(db, "questions_partA");
            ...
            // Compare the existing data with the data you want to upload
            const newData = questionDataPartA.filter((question) => {
            ...
            if (newData.length > 0) {
                const batch = [];
                questionDataPartA.forEach((question) => {
    ...
    return (
        <>
            <button onClick={uploadQuestions}>Initialize Questions Part A in DB</button>
        </>
    )
}
